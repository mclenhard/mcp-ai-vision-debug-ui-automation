...
class AIVisionDebugServer {
     ...
     */
    async screenshotUrl(url, fullPage = false, waitForSelector, waitTime = 1000) {
        try {
            ...
            switch (request.params.name) {
                case 'screenshot_url': {
                    try {
                        ...
                        // Take screenshot
                        const screenshot = await this.screenshotUrl(args.url, args.fullPage || false, args.waitForSelector, args.waitTime || 1000);
                        return {
                            content: [
                                {
                                    type: 'text',
                ...
                case 'display_last_screenshot': {
                    try {
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to display screenshot: ${error?.message || 'Unknown error'}`);
                    }
                }
                case 'batch_screenshot_urls': {
                    try {
                        ...
                        // Set up batch items
                        const batchItems = urls.map((url, index) => ({
                            url,
                        ...
                        for (const item of batchItems) {
                            try {
                                const screenshot = await this.screenshotUrl(item.url, args.fullPage || false, undefined, args.waitTime || 1000);
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to capture batch screenshots: ${error?.message || 'Unknown error'}`);
                    }
                }
                case 'screenshot_local_files': {
                    try {
                        ...
                        const batchItems = fileUrls.map((url, index) => ({
                            url,
                        ...
                        for (const item of batchItems) {
                            try {
                                const screenshot = await this.screenshotUrl(item.url, args.fullPage || false, undefined, 1000);
                                item.screenshotPath = screenshot.path;
                                item.base64Data = screenshot.base64Data;
                            }
                            catch (error) {
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to capture local HTML screenshots: ${error?.message || 'Unknown error'}`);
                    }
                }
                default:
     ...
     */
    async findHtmlFiles(directory, pattern = '*.html') {
        try {
